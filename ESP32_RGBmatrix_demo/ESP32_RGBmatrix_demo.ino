...
const char* ssid1 = "mY iPhone";
const char* password1 = "000111";
...
void setup() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    DBG_OUTPUT_PORT.println("WiFi not connected. Please check your setup!");
...
void handleFileUpload() { // upload a new file to the SPIFFS
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    if (fsUploadFile) {                                   // If the file was successfully created
      ...
      //server.send(303);                                   // HTTP Redirection code
      server.send(200, "text/plain", "200: upload success");
      readBinToBuffer();
      StartTimer();
    } else {
...
void readBinToBuffer() {
  ...
  //uint8_t buf[binSize];
  uint8_t *buf = new uint8_t[binSize];
  ...
  for (unsigned int i = 0; i < ROWS * COLUMNS; ++ i)
  {
    matrix.drawPixel(i % COLUMNS, i / ROWS, buf[i * 3 + 0], buf[i * 3 + 1], buf[i * 3 + 2]);
  }
  // delete
  delete buf;
...
void ResetImage() {
  ...
  //  else
  //    for (int i = 0; i < dim; i++) {
  //      DemoImg[i] = pgm_read_byte(&noWifi[i]);
  //    }
  //  drawImg();
}
